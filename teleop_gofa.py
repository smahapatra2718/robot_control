"""
viser + pyroki teleop scaffold for an ABB GoFa CRB 15000 (variant 12_127).

Mirrors teleop_ur15.py with two differences:
  - State polling uses Robot Web Services (RWS) on the OmniCore controller
    instead of ur_rtde. RWS gets us ~10 Hz, vs UR's 500 Hz, so the viz lags
    a little more behind the real arm.
  - Execute-on-robot uses MoveAbsJ commands via a small RAPID program on
    the controller (see ABB_RAPID_TEMPLATE below). It is NOT a real-time
    stream — the speed slider only affects the in-browser viz playback.
    The robot moves at TCP speed v_tcp set in RAPID.

When/if EGM is licensed, replace the body of _execute_segment() with a UDP
streaming loop and the speed slider becomes unified.

Run:
  ./robot_control/bin/python teleop_gofa.py
"""

import os
import sys
import threading
import time

import jax.numpy as jnp
import jaxlie
import numpy as np
import pyroki as pk
import viser
import yourdfpy
from viser.extras import ViserUrdf

import abb_rws

_HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _HERE)
import pyroki_snippets as pks  # noqa: E402

# ---------- config ----------
ROBOT_IP = "192.168.125.1"      # GoFa MGMT port (direct Mac connection)
RWS_USER = "Default User"
RWS_PASSWORD = "robotics"
URDF_PATH = os.path.join(_HERE, "crb15000_5_95.urdf")
URDF_MESH_DIR_PREFIX = os.path.join(_HERE, "abb_desc")  # used to resolve package:// URIs at load time
TARGET_LINK = "tool0"

RAPID_TASK = "T_ROB1"
RAPID_TARGET_VAR = "py_target"  # jointtarget in your RAPID module
RAPID_GO_FLAG_VAR = "py_go"     # bool in your RAPID module

POLL_HZ = 10                    # RWS practical rate
PLAY_HZ = 60                    # viz refresh when idle / preview
STREAM_HZ = 50                  # viz playback during a Play
MAX_JOINT_SPEED = 1.0           # rad/s peak per joint at slider=1.0 (viz only)
MIN_SEG_DURATION_S = 0.5
DWELL_S = 0.2
RAMP_FRAC = 0.25
EXECUTE_SETTLE_S = 0.5          # extra wait after each RAPID move before next one


def _alpha_to_s(alpha: float, r: float = RAMP_FRAC) -> float:
    """Trapezoidal velocity profile alpha -> traversed fraction."""
    v_peak = 1.0 / (1.0 - r)
    if alpha < r:
        return 0.5 * v_peak * alpha * alpha / r
    if alpha < 1.0 - r:
        return 0.5 * v_peak * r + v_peak * (alpha - r)
    return 1.0 - 0.5 * v_peak * (1.0 - alpha) ** 2 / r


# ---------- robot model ----------
def _resolve_mesh(fname: str) -> str:
    if fname.startswith("package://"):
        pkg, rest = fname[len("package://") :].split("/", 1)
        return os.path.join(URDF_MESH_DIR_PREFIX, pkg, rest)
    return fname


urdf = yourdfpy.URDF.load(URDF_PATH, filename_handler=_resolve_mesh)
robot = pk.Robot.from_urdf(urdf)
TARGET_LINK_IDX = robot.links.names.index(TARGET_LINK)
NUM_JOINTS = robot.joints.num_actuated_joints

# ---------- shared state ----------
state_lock = threading.Lock()
current_q = np.zeros(NUM_JOINTS)
last_poll_ok = False
waypoints: list[tuple[np.ndarray, np.ndarray]] = []
waypoint_frames: list = []
plan_segments: list[tuple[np.ndarray, np.ndarray]] | None = None
playing = threading.Event()
stop_flag = threading.Event()

# ---------- RWS ----------
rws = abb_rws.RWSClient(host=ROBOT_IP, user=RWS_USER, password=RWS_PASSWORD)

# Grab mastership so we can write RAPID variables. atexit in abb_rws releases it.
try:
    rws.request_mastership()
    print("RAPID mastership acquired.")
except Exception as e:
    print(f"WARNING: could not acquire RAPID mastership: {e}")
    print("Execute-on-robot will fail with 403. Resolve by:")
    print("  - releasing mastership from any other RWS client (RobotStudio, prior runs)")
    print("  - or rebooting the controller from the pendant")

# Safety: pre-load py_target with current joints so a stray py_go=TRUE is a no-op.
try:
    current = rws.get_joints()
    rws.set_jointtarget("py_target", current)
    rws.set_rapid_bool("py_go", False)
    print(f"Safety init: py_target = current pose, py_go = FALSE.")
except Exception as e:
    print(f"Safety init failed: {e}")


def poll_loop() -> None:
    global current_q, last_poll_ok
    period = 1.0 / POLL_HZ
    while True:
        try:
            q = np.array(rws.get_joints(), dtype=np.float64)
            with state_lock:
                current_q = q
                last_poll_ok = True
        except Exception:
            with state_lock:
                last_poll_ok = False
        time.sleep(period)


threading.Thread(target=poll_loop, daemon=True).start()
time.sleep(0.4)  # give one RWS roundtrip a chance


def ee_pose(q: np.ndarray) -> jaxlie.SE3:
    Ts = robot.forward_kinematics(cfg=jnp.array(q))
    return jaxlie.SE3(Ts[TARGET_LINK_IDX])


# ---------- viser ----------
server = viser.ViserServer()
server.scene.add_grid("/ground", width=2, height=2)
viser_urdf = ViserUrdf(server, urdf, root_node_name="/base")

with state_lock:
    q0 = current_q.copy()
T_ee0 = ee_pose(q0)
gizmo = server.scene.add_transform_controls(
    "/ee_target",
    scale=0.25,
    position=np.asarray(T_ee0.translation()),
    wxyz=np.asarray(T_ee0.rotation().wxyz),
)

# ---------- GUI ----------
gui_status = server.gui.add_text("Status", initial_value="Idle", disabled=True)
gui_rws_status = server.gui.add_text("RWS", initial_value="?", disabled=True)
gui_wp_count = server.gui.add_text("Waypoints", initial_value="0", disabled=True)
gui_add_wp = server.gui.add_button("Add waypoint (from gizmo)")
gui_pop_wp = server.gui.add_button("Remove last waypoint")
gui_clear_wp = server.gui.add_button("Clear waypoints")
gui_plan = server.gui.add_button("Plan")
gui_play = server.gui.add_button("Play", disabled=True)
gui_stop = server.gui.add_button("Stop", disabled=True)
gui_reset = server.gui.add_button("Reset gizmo to current EE")
gui_speed = server.gui.add_slider("Speed (viz only)", min=0.1, max=2.0, step=0.05, initial_value=1.0)
gui_execute = server.gui.add_checkbox("Execute on robot (RWS / MoveAbsJ)", initial_value=False)


def _refresh_wp_count() -> None:
    gui_wp_count.value = str(len(waypoints))


@gui_add_wp.on_click
def _(_):
    pos = np.asarray(gizmo.position)
    wxyz = np.asarray(gizmo.wxyz)
    waypoints.append((pos, wxyz))
    handle = server.scene.add_frame(
        f"/waypoints/{len(waypoints) - 1}",
        position=pos,
        wxyz=wxyz,
        axes_length=0.12,
        axes_radius=0.005,
    )
    waypoint_frames.append(handle)
    _refresh_wp_count()
    gui_status.value = f"Added waypoint {len(waypoints)}"


@gui_pop_wp.on_click
def _(_):
    if not waypoints:
        return
    waypoints.pop()
    waypoint_frames.pop().remove()
    _refresh_wp_count()


@gui_clear_wp.on_click
def _(_):
    for h in waypoint_frames:
        h.remove()
    waypoint_frames.clear()
    waypoints.clear()
    _refresh_wp_count()


@gui_reset.on_click
def _(_):
    with state_lock:
        q = current_q.copy()
    T = ee_pose(q)
    gizmo.position = np.asarray(T.translation())
    gizmo.wxyz = np.asarray(T.rotation().wxyz)
    gui_status.value = "Gizmo reset to current EE"


@gui_plan.on_click
def _(_):
    global plan_segments
    targets = waypoints if waypoints else [(np.asarray(gizmo.position), np.asarray(gizmo.wxyz))]
    with state_lock:
        q = current_q.copy()
    segments: list[tuple[np.ndarray, np.ndarray]] = []
    for i, (pos, wxyz) in enumerate(targets):
        try:
            q_next = pks.solve_ik_seeded(
                robot=robot,
                target_link_name=TARGET_LINK,
                target_position=pos,
                target_wxyz=wxyz,
                q_seed=q,
                rest_weight=2.0,
            )
        except Exception as e:
            gui_status.value = f"IK failed at waypoint {i + 1}: {e}"
            return
        segments.append((q.copy(), np.asarray(q_next)))
        q = np.asarray(q_next)
    plan_segments = segments
    total = sum(np.linalg.norm(b - a) for a, b in segments)
    gui_play.disabled = False
    gui_status.value = f"Planned {len(segments)} segment(s), total |Δq|={total:.3f}"


def _execute_segment(q_goal: np.ndarray) -> None:
    """Trigger one MoveAbsJ on the controller and block until it finishes.

    Protocol: set py_target, set py_go=TRUE, then poll py_go until RAPID clears it
    back to FALSE (which happens after `MoveAbsJ` completes).
    """
    rws.set_jointtarget(RAPID_TARGET_VAR, q_goal.tolist(), task=RAPID_TASK)
    rws.set_rapid_bool(RAPID_GO_FLAG_VAR, True, task=RAPID_TASK)
    # Give RAPID a beat to observe py_go=TRUE before we start polling.
    time.sleep(0.1)
    deadline = time.time() + 60.0
    while time.time() < deadline:
        if stop_flag.is_set():
            # Don't try to interrupt MoveAbsJ in flight — just stop scheduling.
            # The current segment will finish; the play loop won't queue more.
            return
        try:
            if rws.get_rapid_data(RAPID_GO_FLAG_VAR).upper() == "FALSE":
                break  # MoveAbsJ completed
        except Exception:
            pass
        time.sleep(0.05)
    time.sleep(EXECUTE_SETTLE_S)


def _post_execute_cleanup() -> None:
    global plan_segments
    for h in waypoint_frames:
        h.remove()
    waypoint_frames.clear()
    waypoints.clear()
    _refresh_wp_count()
    with state_lock:
        q = current_q.copy()
    T = ee_pose(q)
    gizmo.position = np.asarray(T.translation())
    gizmo.wxyz = np.asarray(T.rotation().wxyz)
    plan_segments = None
    gui_play.disabled = True


def _play() -> None:
    assert plan_segments is not None
    execute = gui_execute.value
    if execute:
        gui_execute.value = False
    dt = 1.0 / STREAM_HZ
    completed = False

    playing.set()
    stop_flag.clear()
    gui_play.disabled = True
    gui_stop.disabled = False

    try:
        for seg_idx, (q_start, q_goal) in enumerate(plan_segments):
            if stop_flag.is_set():
                break
            delta = q_goal - q_start
            seg_duration = max(MIN_SEG_DURATION_S, float(np.max(np.abs(delta))) / MAX_JOINT_SPEED)
            gui_status.value = f"Segment {seg_idx + 1}/{len(plan_segments)} (viz)"

            # 1) Smooth in-browser preview from q_start to q_goal at slider speed.
            alpha = 0.0
            while alpha < 1.0:
                if stop_flag.is_set():
                    break
                speed = float(gui_speed.value)
                eased = _alpha_to_s(alpha)
                viser_urdf.update_cfg(q_start + delta * eased)
                time.sleep(dt)
                alpha = min(1.0, alpha + dt * speed / seg_duration)

            # 2) Then commit the segment endpoint to the real robot.
            if execute and not stop_flag.is_set():
                gui_status.value = f"Segment {seg_idx + 1}/{len(plan_segments)} (robot)"
                _execute_segment(q_goal)

            if not stop_flag.is_set() and seg_idx < len(plan_segments) - 1:
                time.sleep(max(0.0, DWELL_S / max(0.1, float(gui_speed.value))))

        completed = not stop_flag.is_set()
    finally:
        playing.clear()
        gui_stop.disabled = True
        gui_status.value = "Stopped" if stop_flag.is_set() else "Done"

    if execute and completed:
        time.sleep(0.5)  # let RWS poll catch up
        _post_execute_cleanup()
    else:
        gui_play.disabled = plan_segments is None


@gui_play.on_click
def _(_):
    if plan_segments is None:
        return
    threading.Thread(target=_play, daemon=True).start()


@gui_stop.on_click
def _(_):
    stop_flag.set()


def viz_loop() -> None:
    period = 1.0 / PLAY_HZ
    while True:
        if not playing.is_set():
            with state_lock:
                q = current_q.copy()
                ok = last_poll_ok
            viser_urdf.update_cfg(q)
            gui_rws_status.value = "OK" if ok else "DISCONNECTED"
        time.sleep(period)


threading.Thread(target=viz_loop, daemon=True).start()

print("viser running. Open the URL printed above in a browser.")
print(f"RWS target: http://{ROBOT_IP}  user={RWS_USER!r}")
while True:
    time.sleep(1.0)


# ---------------------------------------------------------------------------
# RAPID module to load on the controller (via RobotStudio -> T_ROB1 -> Modules):
#
#   MODULE PyExec
#     VAR jointtarget py_target := [[0,0,0,0,0,0],[9E9,9E9,9E9,9E9,9E9,9E9]];
#     PERS bool       py_go     := FALSE;
#     CONST speeddata v_tcp     := v200;       ! adjust to taste
#
#     PROC main()
#       WHILE TRUE DO
#         WaitUntil py_go = TRUE;
#         MoveAbsJ py_target, v_tcp, fine, tool0;
#         py_go := FALSE;
#       ENDWHILE
#     ENDPROC
#   ENDMODULE
#
# Workflow on the pendant:
#   1. Auto mode + motors on.
#   2. PP to main, then Start. The program parks at WaitUntil py_go.
#   3. teleop_gofa.py with Execute checked will set py_target and pulse py_go.
# ---------------------------------------------------------------------------
ABB_RAPID_TEMPLATE = """\
MODULE PyExec
  VAR jointtarget py_target := [[0,0,0,0,0,0],[9E9,9E9,9E9,9E9,9E9,9E9]];
  PERS bool       py_go     := FALSE;
  CONST speeddata v_tcp     := v200;

  PROC main()
    WHILE TRUE DO
      WaitUntil py_go = TRUE;
      MoveAbsJ py_target, v_tcp, fine, tool0;
      py_go := FALSE;
    ENDWHILE
  ENDPROC
ENDMODULE
"""
